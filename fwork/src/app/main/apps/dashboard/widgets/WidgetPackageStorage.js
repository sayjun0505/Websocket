...
const WidgetPackageStorage = (props) => {
  const { limit, total } = props.data;
  const [awaitRender, setAwaitRender] = useState(true);

  const colors = ['#805AD5', '#B794F4'];

  useEffect(() => {
    setAwaitRender(false);
  }, []);

  if (awaitRender || !limit || !total) {
    return null;
  }
  return (
    <Paper className="flex flex-col flex-auto shadow border  rounded-2xl overflow-hidden p-24">
      ...
      <div className="flex flex-col flex-auto mt-24 h-96">
        <div className="text-center py-12 flex flex-row items-baseline justify-center">
          <Typography
            className="text-7xl sm:text-8xl font-semibold leading-nonetracking-tighter"
            sx={{ color: colors[0] }}
          >
            {/* {((total / limit) * 100).toFixed(2)} */}
            {Math.round(parseFloat((total / limit) * 100) * 100) / 100}
          </Typography>
        ...
        <div className="-my-12 divide-y">
          <div className="grid grid-cols-2 py-12" key={0}>
            <div className="flex items-center">
              ...
              <Typography className="ml-12 truncate">total</Typography>
            </div>
...
export default memo(WidgetPackageStorage);
