...
const CardAttachment = (props) => {
  ...
  const fileExt = props.item.src.substring(props.item.src.lastIndexOf('.') + 1);
          ...
          <div className="flex items-center justify-center min-w-128 w-128 h-128">
            <Paper className="overflow-hidden shadow">
              {(fileExt === 'jpg' || fileExt === 'gif' || fileExt === 'png') && (
                <ImageViewer src={props.item.src} />
              )}
              {(fileExt === 'doc' || fileExt === 'docx' || fileExt === 'pdf') && (
                <DocViewer src={props.item.src} fileExt={fileExt} />
              )}
              {fileExt === 'mp4' && <VideoViewer src={props.item.src} />}
            </Paper>
          </div>
