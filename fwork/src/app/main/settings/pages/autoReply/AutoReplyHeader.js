...
import { removeAutoReply, saveAutoReply } from '../../store/autoReplySlice';

const AutoReplyHeader = (props) => {
  ...
  function handleSaveReply() {
    ...
    dispatch(saveAutoReply(reply)).then((action) => {
      // console.log('🤖 [ReplyHeader] reset: ', reply);
      reset(reply);
      navigate(`/settings/auto-reply/${action.payload.id || reply.id}`);
    });
  }

  function handleRemoveReply() {
    dispatch(removeAutoReply(id)).then(() => {
      handleClose();
  ...
  return (
    <div className="flex flex-col sm:flex-row flex-1 w-full items-center justify-between space-y-8 sm:space-y-0 py-32 px-24 md:px-32 bg-white">
      <div className="flex flex-col items-center sm:items-start space-y-8 sm:space-y-0 w-full sm:max-w-full min-w-0">
        ...
        >
          ...
          >
            ...
            <span className="flex mx-4 font-medium">Auto Reply</span>
          </Typography>
        </motion.div>
        ...
        <div className="flex items-center max-w-full">
          ...
          >
            ...
            <Typography variant="caption" className="font-medium">
              Generate personalized messages to automatically respond to your users.
            </Typography>
          </motion.div>
        </div>
      </div>
      ...
      >
        <DialogTitle id="alert-dialog-title">Delete Auto Reply?</DialogTitle>
        <DialogContent>
...
export default AutoReplyHeader;
