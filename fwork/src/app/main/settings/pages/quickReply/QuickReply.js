...
const QuickReply = (props) => {
  ...
  const reply = useSelector(selectQuickReply);
  ...
  useDeepCompareEffect(() => {
    function updateReplyState() {
      ...
      // console.log('🤖 Quick Reply id ', id);
      if (id === 'new') {
        dispatch(newQuickReply());
      } else {
        dispatch(getQuickReply(id)).then((action) => {
          if (!action.payload) {
            setNoReply(true);
          }
        });
  ...
  useEffect(() => {
    if (form) {
      // console.log('🤖 Watch Quick Reply ', form);
    }
  }, [form]);
    ...
    return (
      ...
      >
        <Typography color="text.secondary" variant="h5">
          There is no such quick reply!
        </Typography>
        <Button
          className="mt-24"
          component={Link}
          variant="outlined"
          to="/setting/quick-reply"
          color="inherit"
        >
          Go to Quick Replies Page
        </Button>
      </motion.div>
    );
  }
